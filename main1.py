...
class GeneticAlgorithm:
    ...
    def run(self, selection=0, crossover=0, mutation=0, metric="manhattan"):
        ...
        for gen in range(self.generations):
            ...
            if crossover == 1:
                self.population = Uniform(self.population)
            if crossover == 2:
...
def Base(selection=0, crossover=0, mutation=0, metric="manhattan"):
    ...
    ga = GeneticAlgorithm(env, population_size=100, generations=300)
...
def statistical_mode(runs=100, search=0, top_n=5,  metric="radial"):
